...
export async function ValidateAPIKey(apiKey: string) {
  try {
    ...
    if (
      userData.subscriptions.status !== planStatusDecoded ||
...
export function checkIfIsAPIKey(token: string) {
    const pattern = /^[a-z][a-z0-9]{22,24}-[A-Za-z0-9+\]+\/[A-Za-z0-9+\]+-[A-Za-z0-9+\]+\/[A-Za-z0-9+\]+\/[A-Za-z0-9+\/=]+$/;
