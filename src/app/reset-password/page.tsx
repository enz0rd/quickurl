...
export default function Page() {
  ...
  const onError = (errors: any) => {
    if (errors.newPassword) {
      toast.error(
        errors.newPassword.message || "Please enter a valid password",
        {
          duration: 3000,
    ...
    <div className="grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20 font-[family-name:var(--font-geist-sans)]">
      ...
      <main className="flex flex-col gap-[32px] row-start-2 items-center sm:items-start w-[80%] max-w-[500px]">
        <div className="flex flex-col gap-2 items-center">
          ...
          >
            please enter your new password and confirm it below. Make sure to
            use a strong password that you haven't used before.
          </motion.p>
          <FormProvider {...methods}>
            <form
              onSubmit={methods.handleSubmit(onSubmit, onError)}
              className="flex mt-3 flex-col gap-4 w-full max-w-[300px]"
            >
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                <label
                  htmlFor="newPassword"
                  className="text-sm text-zinc-300 font-semibold pl-2"
                >
                  new password
                </label>
                <Input
              ...
              >
                <Button
                  className="bg-zinc-200 text-zinc-900 cursor-pointer w-full hover:bg-zinc-300"
                  type="submit"
