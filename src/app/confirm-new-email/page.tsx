...
export default function Page() {
    ...
    const handleEmailConfirm = async (data: FormValues) => {
        ...
        try {
            ...
            if (token) {
                const res = await fetch("/api/user/confirm-new-email", {
                    method: "POST",
                    headers: {
                    ...
                    {isSubmitting ? (
                        <>
                            <h1 className="text-4xl font-bold text-center">
                                {loading ? (<div className="flex gap-3 items-center"><Loader className="w-12 h-12 animate-spin my-auto" /><span className="my-auto">confirming email</span></div>) : "email changed successfully"}
                            </h1>
                            <p className="text-gray-500 text-md mx-2 text-wrap text-center">
                                {loading ?
                                    "Please wait while we validate your new email..." :
                                    "Your email has been successfully changed! You will be redirected to the login page shortly."
                                }
                            </p>
                        </>
                    ) : (
