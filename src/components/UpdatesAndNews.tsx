...
export default function UpdatesAndNews() {
  ...
  useEffect(() => {
    ...
    if (!lastChecked.date || new Date(lastChecked.date) < updateDate && !lastChecked.content) {
      fetchData();
      setIsOpen(true);
      const existingData = JSON.parse(localStorage.getItem("updates") || "{}");
      localStorage.setItem(
        "updates",
        JSON.stringify({ ...existingData, date: updateDate })
      );
      if (existingData.content) {
        setSource(existingData.content);
      }
      setIsChecked(true);
    }

    if (lastChecked.content) {
      setSource(lastChecked.content);
    }
  ...
  const fetchData = async () => {
    try {
      ...
      const data = await fetch(
        "https://raw.githubusercontent.com/enz0rd/quickurl/refs/heads/master/changelog.md",
        {
          method: "GET",
        }
      );
      ...
      localStorage.setItem(
        ...
        JSON.stringify({
          ...
          content: text.substring(0, 500) + "...\n\n ### see more at [GitHub Changelog](https://github.com/enz0rd/quickurl/blob/master/changelog.md)",
        })
      );
      setSource(text);
    } catch (error) {
      toast.error(
        "An error occurred while trying to get updates. Please try again later.",
        {
          duration: 5000,
