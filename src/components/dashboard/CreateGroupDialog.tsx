...
const formSchema = z.object({
  name: z
    .string()
    .min(6, { message: "Group name must have at least 6 characters" })
    .max(30, { message: "Group name must be 30 characters or less" }),
  description: z
    .string()
    .max(100, { message: "Group description must be 100 characters or less" })
    .optional(),
  shortName: z
    .string()
    .min(1, { message: "Please enter a group short name" })
    .max(4, { message: "Short name must be 4 characters or less" }),
});
  ...
  async function handleCreateGroup(data: FormData) {
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(
          result.error || "Failed to create group. Please try again later.",
          {
            duration: 5000,
            position: "bottom-center",
            icon: "🚫",
        ...
        >
          <AlertDialogHeader className="flex flex-row justify-between">
            <div className="flex flex-col gap-2 justify-start items-start">
              ...
              <AlertDialogDescription className="text-sm text-zinc-500 text-start">
          ...
          <form
            ...
            className="flex flex-col mt-2 w-[250px] md:w-[300px]"
          >
            <div className="flex md:flex-row flex-col gap-3 w-full">
              <div className="flex flex-col gap-1 md:w-[60%]">
                <label htmlFor="name" className="text-sm text-zinc-500 pl-2">
                  group name *
                </label>
                <Input
                  type="text"
                  ...
                  {...register("name")}
                />
                <small className="text-zinc-400 self-end pr-2">
                  30 characters max
                </small>
                {errors.name && (
              ...
              <div className="flex flex-col gap-1 md:w-[40%]">
                <label
                  htmlFor="shortName"
                  className="text-sm text-zinc-500 pl-2"
                >
                  short name *
                </label>
                <Input
            ...
            <AlertDialogFooter className="flex flex-row justify-between w-full mt-5 items-center">
              <span
                className="bg-transparent border-none text-zinc-400 cursor-pointer mr-auto ml-2 text-sm hover:text-zinc-200 transition 1s"
                onClick={() => setOpen(false)}
              >
                cancel
              </span>
